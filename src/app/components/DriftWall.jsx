...
const DriftWall = ({
  ...
  const handleTileClick = useCallback((id, colIndex, e) => {
    e.stopPropagation();
  ...
  const renderTile = (item, id, colIndex) => {
    ...
    const inner = (
      <span className="drift-wall__inner">
        <img src={item.image} alt={item.title ?? ''} loading="lazy" decoding="async" width={600} height={400} draggable={false} />
        <span className="drift-wall__overlay" aria-hidden="true" />
      </span>
    );
    const commonProps = {
      className: `drift-wall__tile${activeId === id ? ' is-active' : ''}${isFocused ? ' is-focused' : ''}`,
      'data-tile-id': id,
      'data-col': colIndex,
      onClick: (e) => handleTileClick(id, colIndex, e),
